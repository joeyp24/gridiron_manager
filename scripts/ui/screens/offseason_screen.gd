...
func _extension_card() -> PanelContainer:
	...
	values.add_child(_metric("APY", PlayerContract.money_label(contract.annual_salary)))
	values.add_child(_metric("YEAR 1 CAP", PlayerContract.money_label(contract.current_cap_hit())))
	values.add_child(_metric("GUARANTEED", PlayerContract.money_label(contract.total_guaranteed)))
