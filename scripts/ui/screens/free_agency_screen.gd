...
func _rebuild_detail() -> void:
	...
	value_row.add_child(_metric("APY", PlayerContract.money_label(contract.annual_salary)))
	value_row.add_child(_metric("YEAR 1 CAP", PlayerContract.money_label(contract.current_cap_hit())))
	value_row.add_child(_metric("GUARANTEED", PlayerContract.money_label(contract.total_guaranteed)))
