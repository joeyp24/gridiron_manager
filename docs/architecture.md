...
`SaveRepository` writes a versioned JSON envelope around serialized career state. The current schema is version 16. Earlier migrations add contracts and free agency, fixed expirations and potential, history and development reports, rookie draft state, enriched player/career metadata, retirement archives, source provenance, league format, future-season schedule templates, playoff seeds, future draft-pick ownership, trade history, game books, season/career statistics, hybrid Madden ratings, Fantasy Draft state, explicit roster/reserve state, matchup-keyed weekly game plans, persistent team trade blocks, offer inbox/history, and weekly market-cycle markers. Version fifteen adds year-aware contract schedules, guarantee and transaction accounting, and team cap metadata. Version sixteen adds persistent coach progression, seeded AI builds, and deduplicated progression rewards; existing coaches begin with starting points, not retroactive XP. Existing careers receive compatible defaults while eight-team saves retain their smaller legacy format. See [coaching.md](coaching.md) for the skill catalog and bounded simulation hooks.
...
`scripts/ui` contains the centralized theme, reusable controls, responsive route screens, and shell navigation. A persistent left rail owns primary routing, while its compact mode retains every route as a short, tooltip-backed control. The contextual header reflects the active section, season, week, club, and record. Screens render application/domain state and emit user intent through shared page headings, section markers, metric cards, status pills, table surfaces, and action tiers. Career creation selects standard rosters or Fantasy Draft mode; active Fantasy Draft saves route back into the war room until finalization, while other career sections remain locked against incomplete rosters. The Match Center adds an optional responsive offensive call sheet without replacing its automatic snap, drive, or full-game controls. The adjacent `scripts/presentation` layer contains transient `PlayAnimationData` and `PlayActorTrack` timelines; `PlayAnimationComposer` converts completed results into deterministic 22-player movement, and `FieldVisual` draws the top-down replay without feeding presentation state back into simulation or persistence. The Statistics Center uses the application query boundary for sortable leaders, team rankings, player dossiers, game logs, and game books. The shell-owned `SimulationLoadingOverlay` presents actual task progress and blocks navigation while staged league state is incomplete. Wide layouts use multiple columns; narrower layouts reflow into scrollable single-column views instead of relying on a fixed resolution. The visual rules and extension conventions are documented in `docs/ui_design_system.md`.
